...
Writing down the complete configuration of of your resource from sratch to deployment in one code or file.
Saves human error, time, efficient, reduce manually configuration, save resources, reduce dependecies on other teams.
2. What problems does IaC solve compared to manually creating resources in the AWS console?

You can deploy or destroy complete infructure with the simple command 
3. How is Terraform different from AWS CloudFormation, Ansible, and Pulumi?

- In terraform you write the complete configuration in one file , OSS, BSS
- where AWS cloudformation is aws native not OSS
- ANSIBLE (Config management Tool not Iaac )you define the each steps one by one which commands will be executed,Uses procedural steps (imperative)

4. What does it mean that Terraform is "declarative" and "cloud-agnostic"?

Terraform is declarative : describing the desired state. you write and defin the complete resource config in one go which is open source.

Cloud-agnostic : Terraform works across multiple cloud providers, Same tool can manage AWS, Azure, GCP, etc., Avoids vendor lock-in
...
3. Answer these questions in your notes:
   ...
   - Why should you never manually edit the state file?
     it can mess up sync state for resource so let the terraform manage it.
   - Why should the state file not be committed to Git?

     it contains the secrets and keys of your file or resource
