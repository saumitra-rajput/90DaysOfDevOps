...
This time project name was already picked by terraform from the terraform.tfvars files

See the jarvis as project name and dev as environment
...
pass the file name on terraform plan 
...
This means the last value applied wins.
---
...
- Terraform controls its lifecycle (create, update, destroy)
...
Data Source
- Fetches existing information from AWS (read-only)
...
|-----------|----------------------|-------------------|
| Purpose   | Create/manage infra  | Read existing data|
| Lifecycle | Managed by Terraform | Not managed       |
| Example   | EC2, VPC             | AMI, AZs          |
...
Conditional Expression ? : Chooses a value based on a condition.

```var.environment == "prod" ? "t3.small" : "t2.micro"```

No hardcoded names or ports.
Supports multiple environments (dev, prod) via variables.
Easy to maintain and scale (just update locals or variables).
---
